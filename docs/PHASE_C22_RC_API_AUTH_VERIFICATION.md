...
## 10. Current blocker status (2026-05-13)

> C2.5 **заблокирован**. Переход к Apply → RealtyCalendar невозможен до снятия всех блокеров.

| Компонент | Статус | Требуется |
|---|---|---|
| `RC_API_TOKEN` | **missing** — отсутствует в `.env` и n8n | Получить от RC (личный кабинет или поддержка) |
| Auth header format | **unknown** — Bearer / X-Api-Key / двойной? | Подтверждение от RC |
| GET `/prices` endpoint | **unverified** — не тестировался | Ручной curl-тест после получения токена |
| POST `/prices` endpoint | **found, not tested** — найден в workflow 04 (inactive) | Тест только после GET-верификации и явного approve |
| Write access to RC | **forbidden** — запрещено до явного разрешения владельца | Отдельный approve от владельца перед C2.5 |

### Что нужно для разблокировки C2.5

1. `RC_API_TOKEN` — получить у RealtyCalendar
2. Формат auth header — уточнить у RC (Вариант A/B/C из раздела 6)
3. Успешный ручной `GET /api/v1/lots/302043/prices` → HTTP 200 или 404 (не 401/403)
4. Sandbox/test mode **или** явное разрешение владельца на первый тестовый apply с немедленным rollback

До получения всех четырёх пунктов — никаких POST-запросов к RC API.

*Статус блокера зафиксирован 2026-05-13. Никаких изменений в системе не производилось.*

---

## ОБНОВЛЕНИЕ 2026-05-13 — Официальный ответ / Phase закрыта

**Получен официальный ответ RealtyCalendar:**

> "API не предоставляем и нет такой возможности, открытого API у нас также нет."

### Итог Phase C2.2

- **Auth verification** — не нужна: API не существует
- **C2.5 API Apply** — отменён навсегда
- **Все curl-команды для тестирования API** — не применимы и не должны выполняться
- **`RC_API_TOKEN` в env** — не нужен

### Следующий шаг

Phase C2.3 — Semi-manual export. Цены экспортируются как JSON + инструкция, применяются владельцем вручную в интерфейсе RealtyCalendar. Документ: `docs/PHASE_C23_RC_SEMI_MANUAL_EXPORT_PLAN.md`
